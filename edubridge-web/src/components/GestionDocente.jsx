...
const QuickAction = ({ title, icon, color, onClick }) => (
    <div onClick={onClick} className={`p-6 rounded-[28px] ${color} flex flex-col items-center justify-center gap-4 cursor-pointer hover:scale-105 transition-all shadow-sm border border-transparent active:scale-95`}>
        <span className="text-3xl">{icon}</span>
        <span className="text-[10px] font-black uppercase text-center leading-tight tracking-tighter">{title}</span>
    </div>
);
...
const ReportItem = ({ task, onClick }) => (
    <div className="space-y-3 group">
        <div className="flex justify-between items-start">
            <div><p className="font-bold text-slate-900 text-sm group-hover:text-blue-600 transition-colors">{task.title}</p><p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">{task.tag} Prioridad</p></div>
            <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase shadow-sm ${task.status === 'Pendiente' ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'}`}>{task.status}</span>
        </div>
        <div className="flex items-center gap-4">
            <div className="flex-1 bg-slate-100 h-2.5 rounded-full overflow-hidden shadow-inner"><div className="bg-blue-600 h-full transition-all duration-1000 shadow-md" style={{ width: `${task.progress}%` }}></div></div>
            <span className="text-xs font-black text-slate-700">{task.progress}%</span>
        </div>
        <button onClick={onClick} className="w-full bg-slate-50 text-slate-600 py-2.5 rounded-xl text-[10px] hover:bg-blue-600 hover:text-white transition-all font-black uppercase">Generar Documento</button>
    </div>
);
...
const ScheduleItem = ({ time, course, room, completed }) => (
    <div className={`p-4 rounded-2xl border transition-all text-left ${completed ? 'bg-slate-50 border-slate-100 opacity-60' : 'bg-white border-blue-100 shadow-sm'}`}>
        <p className="text-[10px] font-bold text-blue-600 mb-1">{time}</p>
        <p className="text-xs font-black text-slate-900 leading-none">{course}</p>
        <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">{room}</p>
    </div>
);

const StatBar = ({ label, value, color }) => (
    <div className="space-y-2 text-left">
        <div className="flex justify-between items-center text-[10px] font-black text-slate-500 uppercase"><span>{label}</span><span>{value}%</span></div>
        <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden shadow-inner"><div className={`${color} h-full transition-all duration-[1500ms] shadow-lg`} style={{ width: `${value}%` }}></div></div>
    </div>
);
